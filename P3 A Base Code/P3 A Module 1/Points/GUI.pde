...
void keyPressed()  // executed each time a key is pressed: sets the Boolean "keyPressed" till it is released   
    ...
    if(key=='1') switchMode = true;               // toggles what should be displayed at each fram
    if(key=='2') switchMode = false;
    if(key=='3') ;
    if(key=='4') ;
    if(key=='5') ;
    if(key=='6') ;
...
String title ="Tango Dancer: Project 3 Part A: caplet ",            name ="Amy Zhuang",
